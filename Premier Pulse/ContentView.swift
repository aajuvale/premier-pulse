...
struct ContentView: View {
    ...
    @State private var upcomingMovies: [Movie] = []
    @State private var favorites: [Movie] = []
    @State private var showingFavorites = false // Controls the favorites sheet
        ...
        NavigationView {
            ZStack {
                ...
                VStack(spacing: 10) {
                    ...
                    // Dynamic Section: Search Results or Upcoming Movies
                    if query.isEmpty {
                        MovieSection(title: "Upcoming Movies", movies: upcomingMovies, favorites: $favorites, addToFavorites: addToFavorites)
                    } else {
                        MovieSection(title: "Search Results", movies: results, favorites: $favorites, addToFavorites: addToFavorites)
                    }
    ...
    // Fetch and utility functions remain the same...
    // fetchMovies, fetchUpcomingMovies, addToFavorites, scheduleNotification, getAPIKey
}

// MARK: - Movie Section with Cards
...
struct MovieCard: View {
    ...
    var body: some View {
        HStack(alignment: .top) {
            ...
            // Animated Add to Favorites Button
            Button(action: {
